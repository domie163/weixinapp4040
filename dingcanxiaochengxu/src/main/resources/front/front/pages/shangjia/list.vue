<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.shangjiaName" type="text" placeholder="商家名称"></input>
		</view>
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.shangjiaPhoto?product.shangjiaPhoto.split(',')[0]:''"></image>
        <view class="listmr">
           ...
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('shangjia','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            ...
            v-if="isAuth('shangjia','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 商家星级类型 字段 字典表查询方法
		  	let shangjiaXingjiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'shangjia_xingji_types'
		  	});
		  	this.shangjiaXingjiTypesList = shangjiaXingjiTypes.data.list;
